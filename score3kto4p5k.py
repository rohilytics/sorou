...
csv_file_path = '/root/sorou-main/news_sentiment_scores.csv'

# Connect to the existing SQLite database
conn = sqlite3.connect('/root/sorou-main/reuters.db')
cursor = conn.cursor()

# Fetch all news articles starting from news_id 6000
cursor.execute("SELECT news_id, headline, text FROM news WHERE news_id >= 3000 AND news_id <= 4500 ORDER BY news_id")
news_articles = cursor.fetchall()

# Create or connect to the new database where the sentiment will be stored
conn_sentiment = sqlite3.connect('/root/sorou-main/news_sentiment.db')
...
with open(csv_file_path, mode='a', newline='') as csv_file:
    ...
    llm = Llama(
        model_path = r"/root/sorou-main/Llama-3-Instruct-8B-SPPO-Iter3-Q5_K_M.gguf",
        n_gpu_layers = -1,
        seed = 123,
        n_ctx = 8192,
        verbose = False
